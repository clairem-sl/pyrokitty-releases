...
class Bot {
    clientEvents;
    stayRegion = '';
    stayPosition = new Vector3_1.Vector3();
    loginParams;
    ping = null;
    pingNumber = 0;
    lastSuccessfulPing = 0;
    circuitSubscription = null;
    options;
    eventQueueRunning = false;
    eventQueueWaits = new Map();
    stay = false;
    /**
    ...
    async login() {
        ...
        const response = await loginHandler.Login(this.loginParams);
    ...
    async changeRegion(region, requested) {
        this.closeCircuit();
    ...
    async waitForEventQueue(timeout = 1000) {
    ...
    async close() {
        ...
        await circuit.waitForMessage(Message_1.Message.LogoutReply, 5000);
        this.stayRegion = '';
    ...
    async connectToSim(requested = false) {
        if (!requested) {
        ...
        await circuit.waitForAck(circuit.sendMessage(msg, PacketFlags_1.PacketFlags.Reliable), 60000);
        ...
        const handshakeMessage = await circuit.waitForMessage(Message_1.Message.RegionHandshake, 10000);
        ...
        await circuit.waitForAck(circuit.sendMessage(handshakeReply, PacketFlags_1.PacketFlags.Reliable), 10000);
        ...
        this.ping = setInterval(() => {
            (async () => {
                ...
                await circuit.waitForMessage(Message_1.Message.CompletePingCheck, 10000, ((pingData, cpc) => {
